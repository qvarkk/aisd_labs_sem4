...
Graph Graph::Union(Graph G1, Graph G2) {
    if (G1._Size != G2._Size)
        return Graph(0);

    Graph G(G1._Size);
    for (int i = 0; i < G1._Size; i++) {
        for (int j = 0; j < G1._Size; j++)
            if (G1._m[i][j] < G2._m[i][j]) {
                G._m[i][j] = G1._m[i][j];
            } else {
                G._m[i][j] = G2._m[i][j];
            }
...
// Нахождение длины пути (если граф представляет из себя путь)
double Graph::PathLength() {
    double len = 0;
    for (int i = 0; i < _Size; i++) {
        for (int j = 0; j < _Size; j++) {
            if (_m[i][j] != DBL_MAX)
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "City A: " << cityA << std::endl;
    std::cout << "City B: " << cityB << std::endl;
    std::cout << "City C: " << cityC << std::endl;

    g.Random(1, 5);
    g.Print();
    Graph g1 = g.ShortestPath(cityA, cityC);
    Graph g2 = g.ShortestPath(cityC, cityB);
    if (g1.EdgeCount() != 0 && g2.EdgeCount() != 0) {
        Graph path = Graph::Union(g1, g2);
        path.Print();
        path.PrintPath(cityA, cityB);
        std::cout << "Path Length: " << path.PathLength() << std::endl;
    } else {
        std::cout << "[ERROR] Cannot find path from A to B through C" << std::endl;
    }
